...
- Tartalmi jelölések: `:::page type=… level=… disc=…`, `:::you`, `:::protocol`, `:::figure src=… caption=… ev=…`; inline `{ev:A}`–`{ev:D}`, `{fix}` / `{példa}` / `{egyéni}` / `{férfi}` (csak férfi adat), `[@kulcs]`; oldaltípusok között `vita` is (eldöntetlen szakmai vita). Részletek: `design/styleguide.md` 7. pont.
